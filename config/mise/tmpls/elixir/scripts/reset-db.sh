...
dev | test) ;;
*)
  echo "error: refusing database reset with MIX_ENV=${MIX_ENV}" >&2
  exit 2
  ;;
esac
...
[ -n "${DATABASE_URL:-}" ] && {
  echo 'error: DATABASE_URL must be unset for db:reset' >&2
  exit 2
}
[ "${PG_ROOT:-}" = "$expected_root" ] || {
  echo "error: PG_ROOT must be $expected_root" >&2
  exit 2
}
[ "${PGDATA:-}" = "$expected_root/data" ] || {
  echo "error: PGDATA must be $expected_root/data" >&2
  exit 2
}
[ -z "${PGHOST:-}" ] || [ "$PGHOST" = "$expected_root" ] || {
  echo "error: PGHOST must be unset or $expected_root" >&2
  exit 2
}
[ -z "${PGSERVICE:-}" ] || {
  echo 'error: PGSERVICE must be unset for db:reset' >&2
  exit 2
}
expected_port="$(bash "$(dirname "$0")/pg-port.sh")"
[ -z "${PGPORT:-}" ] || [ "$PGPORT" = "$expected_port" ] || {
  echo "error: PGPORT must be unset or $expected_port" >&2
  exit 2
}
