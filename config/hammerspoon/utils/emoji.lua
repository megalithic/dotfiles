...
M.codepoints = function(str)
  ...
  for _, codepoint in utf8.codes(str) do
    ...
    if emojiSingles[codepoint] then
      insert = true
    else
      -- Check all ranges
      for _, range in ipairs(emojiRanges) do
    ...
    if insert then table.insert(codepoints, codepoint) end
  end
...
M.has_emoji = function(str)
  local codepoints = M.codepoints(str)

  return #codepoints > 0
end

M.parse_codepoints = function(str)
  return hs.fnutils.map(hs.fnutils.split(str, "-"), function(hex) return tonumber(hex, 16) end)
end

M.stringify_codepoints = function(codepoints) return hs.utf8.codepointToUTF8(table.unpack(codepoints)) end
return M
