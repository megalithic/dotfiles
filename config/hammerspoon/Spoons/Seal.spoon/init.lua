...
obj.plugin_search_paths = { hs.configdir .. "/lib/seal", hs.configdir .. "/seal_plugins", obj.spoonPath }
...
function obj:bindHotkeys(mapping)
  if self.hotkeyShow then self.hotkeyShow:delete() end
...
function obj:start()
  print("-- Starting Seal")
  if self.hotkeyShow then self.hotkeyShow:enable() end
...
function obj.choicesCallback()
  ...
  cmd = nil
  query_words = {}
  if tostring(query):find("^%s*$") ~= nil then return choices end
...
function obj.queryChangedCallback(query)
  if obj.queryChangedTimer then obj.queryChangedTimer:stop() end
  obj.queryChangedTimer = hs.timer.doAfter(
    obj.queryChangedTimerDuration,
