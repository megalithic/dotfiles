...
local function appendToHistory(entry)
  ...
  if not ok then U.log.wf("error appending to history: %s", tostring(err)) end
end
  ...
  local ok, err = pcall(function()
    ...
    for file in iter, dir do
      if file:match("%.jsonl$") and file ~= currentFile then
        ...
        if success then U.log.df("archived %s", file) end
      end
    end
  end)

  if not ok then U.log.wf("error archiving history: %s", tostring(err)) end
end
...
local function sendCommand(command)
  ...
  local ok, err = pcall(function() M.process:setInput(jsonStr .. "\n") end)
...
local function recordActivity() M.lastActivityTime = os.time() end
...
local function updateProviderState(stateData)
  local ok, err = pcall(function()
    ...
    if M.isUsingFallback and not wasFallback then
      U.log.wf("FALLBACK DETECTED - now using %s/%s (was %s/%s)", provider, modelId, M.initialProvider, M.initialModel)
  ...
  if not ok then U.log.wf("error in updateProviderState: %s", tostring(err)) end
end
...
local function abortTask(reason)
  ...
  if not ok then U.log.wf("error in abortTask: %s", tostring(err)) end
end
...
local function startTaskTimer()
  local ok, err = pcall(function()
    ...
    if M.taskTimer then pcall(function() M.taskTimer:stop() end) end
    ...
    M.taskTimer = hs.timer.doAfter(timeoutSeconds, function()
      ...
      if not checkOk then U.log.wf("error in task timer callback: %s", tostring(checkErr)) end
    end)
  end)

  if not ok then U.log.wf("error starting task timer: %s", tostring(err)) end
end
...
local function isAbortCommand(text)
  ...
  for _, phrase in ipairs(cfg.abortPhrases) do
    if lower == phrase:lower() then return true end
  end
  return false
end
...
local function getQueueStatusText()
  ...
  if total == 0 then
    ...
    if priorityCount > 0 then table.insert(lines, string.format("  ⚡ %d priority", priorityCount)) end
    if normalCount > 0 then table.insert(lines, string.format("  📥 %d normal", normalCount)) end
  end
...
local function handleEvent(event)
    ...
    if event.type == "response" then
      ...
      if event.success then
        ...
        if event.command == "get_state" then
          ...
          if event.data then pcall(updateProviderState, event.data) end
        end
      else
    ...
    elseif event.type == "agent_end" then
      ...
      if event.messages and type(event.messages) == "table" then
        ...
        if responseText and type(responseText) == "string" and #responseText > 0 then
          ...
          if indicator and #indicator > 0 then responseText = responseText .. "\n\n—" .. indicator end
  ...
  if not ok then U.log.wf("error handling event: %s", tostring(err)) end
end
...
local function onOutput(task, stdout, stderr)
  ...
  local ok, err = pcall(function()
    ...
    if stderr and type(stderr) == "string" and #stderr > 0 then U.log.wf("PiGateway stderr: %s", stderr:sub(1, 500)) end
    ...
    if iterations >= maxIterations then U.log.w("hit max iterations processing output") end
  end)

  if not ok then U.log.wf("error in onOutput: %s", tostring(err)) end
  ...
  local ok, err = pcall(function()
    ...
    if not M.inCooldown then
      ...
      M.restartTimer = hs.timer.doAfter(RESTART_DELAY_SECONDS, function()
        ...
        if not startOk then U.log.wf("auto-restart failed: %s", tostring(startErr)) end
      end)
    end
  end)

  if not ok then U.log.wf("error in onExit: %s", tostring(err)) end
end
  ...
  local ok, err = pcall(function()
    ...
    if (M.consecutiveFailures or 0) >= threshold then
      ...
      M.cooldownTimer = hs.timer.doAfter(cooldownSecs, function()
        ...
        if not resetOk then U.log.wf("error resetting after cooldown: %s", tostring(resetErr)) end
      end)

      -- Stop current process (safely)
      pcall(function()
        if M.process and M.process:isRunning() then M.process:terminate() end
      end)
    end
  end)

  if not ok then U.log.wf("error in checkCircuitBreaker: %s", tostring(err)) end
end
...
local function processNextInQueue()
  local ok, err = pcall(function()
    ...
    if not msg then msg = table.remove(M.queue.normal, 1) end
...
local function queueMessage(text, isPriority)
  local ok, result = pcall(function()
    ...
    if not M.busy then processNextInQueue() end
...
local function handleUnhealthy()
  ...
  if not ok then U.log.wf("error in handleUnhealthy: %s", tostring(err)) end
end
...
local function healthCheck()
  local ok, err = pcall(function()
    ...
    local isRunning = false
    if M.process then pcall(function() isRunning = M.process:isRunning() end) end

    if not isRunning then
      U.log.w("health check failed - process not running")
      -- Only restart if not in cooldown
      if not M.inCooldown then M.start() end
      return
    end
  ...
  if not ok then U.log.wf("error in healthCheck: %s", tostring(err)) end
end
...
function M.start()
  ...
  if result then
    ...
    safeTelegramSend("🤖 Pi gateway ready")
  end
...
function M.stop()
  ...
  if not ok then U.log.wf("error in stop: %s", tostring(err)) end
end
...
---@return boolean
function M.isBusy() return M.busy or false end
...
function M.init()
  ...
  if not ok then U.log.wf("error in init: %s", tostring(err)) end
end
...
function M.cleanup()
  ...
  if not ok then U.log.wf("error in cleanup: %s", tostring(err)) end
end
