...
local SEARCH_PATHS = {
  ...
  "/System/Applications",
  "~/Applications",
  -- "/nix/store/a0qaly4bc5g6ml49jr76daz915mb74l1-home-manager-applications/Applications",
...
Item.is_pref_pane = function(item)
  return item.kMDItemPath ~= nil and item.kMDItemPath and string.find(item.kMDItemPath, "%.prefPane$")
end
...
Item.get_display_name = function(item)
  ...
  if display_name == nil and item.kMDItemPath then display_name = hs.fs.display_name(item.kMDItemPath) end
  ...
  return ("%s%s"):format(display_name, is_pref_pane_item and " preferences" or "")
end

Item.get_icon = function(item)
  local is_pref_pane_item = Item.is_pref_pane(item)
  if is_pref_pane_item then return hs.image.iconForFile(item.kMDItemPath) end

  local bundleID = item.kMDItemCFBundleIdentifier
  if bundleID then return hs.image.imageFromAppBundle(bundleID) end
...
Cache.remove = function(item)
  local display_name = Item.get_display_name(item)
  if display_name then M.results_cache[display_name] = nil end
end
...
Choices.get = function(query, filter)
  ...
  for name, app in pairs(M.results_cache) do
    if string.match(name:lower(), query:lower()) then
      ...
      if should_insert then table.insert(results, choice) end
    end
  end
  return results
end
...
---@return boolean
Choices.filter_revealable = function(params) return params.running end
...
local function spotlight_results_callback(_, msg, spotlight_results)
  ...
  if spotlight_results.kMDQueryUpdateAddedItems then Cache.update(spotlight_results.kMDQueryUpdateAddedItems, true) end
...
function M:bare() return Choices.get end
...
M.completionCallback = function(choice)
  if not choice["type"] then
    if string.find(choice["path"], "%.applescript$") or string.find(choice["path"], "%.scpt$") then
  ...
  if choice["type"] == "reveal" then
    hs.osascript.applescript(string.format([[tell application "Finder" to reveal (POSIX file "%s")]], choice["path"]))
