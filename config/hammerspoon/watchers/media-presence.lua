...
local function poll()
  ...
  hs.task.new("/bin/sh", function(exitCode, stdOut, _)
    ...
    if not prev then
      prev = p
      U.log.df("[media-presence] initial state: meeting=%s, mic=%s, camera=%s, sharing=%s",
        p.meetingState or "?", tostring(p.micActive), tostring(p.cameraActive), tostring(p.sharing))
      return
    end
