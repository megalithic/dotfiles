...
M.start = function()
  ...
  hs.urlevent.httpCallback = function(scheme, host, _, full_url)
    ...
    local running_browser_name = fn.find(Config.preferred.browsers, function(browser_name)
    ...
    log.f(
      "%s -> %s [%s, %s, focus? %s, %s (frontmost? %s)]",
      current_browser:name(),
      full_url,
      scheme,
      host,
      should_refocus,
      current_app:bundleID(),
      current_app:isFrontmost()
    )
