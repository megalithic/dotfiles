...
# time_color = Color(135, 192, 149)
# utc_color = Color(113, 115, 116)
...
#     utc = datetime.now(datetime.timezone.utc).strftime(" (UTC %H:%M)")
#     cells = get_battery_cells()
#     cells.append((time_color, time))
...
def get_battery_cells() -> list:
    try:
        with open("~/.dotfiles/bin/battery", "r") as f:
            status = f.read()
        print(status)
        # with open("/sys/class/power_supply/BAT0/capacity", "r") as f:
        #     percent = int(f.read())
