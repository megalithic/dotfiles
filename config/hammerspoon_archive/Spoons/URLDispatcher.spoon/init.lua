...
function obj.matchapps(app, pat)
  local ismatch = (pat == nil)
    or (type(pat) == "string" and obj.matchapp(app, pat))
    or (type(pat) == "table" and hs.fnutils.some(pat, hs.fnutils.partial(obj.matchapp, app)))
  if ismatch then
...
function obj:patfileWatcher(patfile, paths, flags)
  ...
  -- by editors
  if hs.fnutils.some(flags, function(f) return f["itemModified"] end) then self:read_and_store(patfile) end
end
...
function obj:setupPatfile(patfile)
  ...
  if hs.fs.attributes(patfile) then
    ...
    self.pat_watchers[patfile] =
      hs.pathwatcher.new(patfile, hs.fnutils.partial(self.patfileWatcher, self, patfile)):start()
...
function obj:dispatchURL(scheme, host, params, fullUrl, senderPid)
  local url = fullUrl
  local currentApp = ""
  if senderPid ~= -1 then currentApp = hs.application.applicationForPID(senderPid):name() end
  ...
  elseif type(self.default_handler) == "function" then
    ...
    self.logger.ef(
      "Unknown type '%s' for default_handler '%s', must be a string or a function.",
      type(self.default_handler),
