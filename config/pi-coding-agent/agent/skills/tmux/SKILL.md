...
**Script location:** `~/.dotfiles/config/pi-coding-agent/agent/skills/tmux/scripts/`
...
- List sessions on your active socket with metadata: `~/.dotfiles/config/pi-coding-agent/agent/skills/tmux/scripts/find-sessions.sh -S "$SOCKET"`; add `-q partial-name` to filter.
- Scan all sockets under the shared directory: `~/.dotfiles/config/pi-coding-agent/agent/skills/tmux/scripts/find-sessions.sh --all` (uses `CLAUDE_TMUX_SOCKET_DIR` or `${TMPDIR:-/tmp}/claude-tmux-sockets`).
...
- Use timed polling to avoid races with interactive tools. Example: wait for a Python prompt before sending code:
  ...
  ```bash
  ~/.dotfiles/config/pi-coding-agent/agent/skills/tmux/scripts/wait-for-text.sh -t "$SESSION":0.0 -p '^>>>' -T 15 -l 4000
  ```
...
`~/.dotfiles/config/pi-coding-agent/agent/skills/tmux/scripts/wait-for-text.sh` polls a pane for a regex (or fixed string) with a timeout. Works on Linux/macOS with bash + tmux + grep.

```bash
~/.dotfiles/config/pi-coding-agent/agent/skills/tmux/scripts/wait-for-text.sh -t session:0.0 -p 'pattern' [-F] [-T 20] [-i 0.5] [-l 2000]
```
