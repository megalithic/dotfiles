...
async function getWsUrl() {
	...
	const ports = [process.env.CDP_PORT];
	if (process.env.CDP_PORT_FILE && existsSync(process.env.CDP_PORT_FILE)) {
		const [port] = readFileSync(process.env.CDP_PORT_FILE, "utf8")
			.trim()
			.split("\n");
	...
	for (const portFile of candidates) {
		if (portFile === process.env.CDP_PORT_FILE || !existsSync(portFile))
			continue;
	...
	for (const port of [...new Set(ports.filter(Boolean))]) {
		...
		try {
			...
			attempts.push(`${versionUrl} (${error.message})`);
		}
	}

	throw new Error(
		`Cannot find remote-debugging browser. Tried: ${attempts.join(", ") || "no ports"}`,
	);
}
