...
export default function (pi: ExtensionAPI) {
      ...
      (tui, theme, _kb, done) => {
        ...
        const doExtract = async () => {
          ...
          if (!auth.ok || !auth.apiKey) {
            throw new Error(
              auth.ok
                ? `No API key for ${extractionModel.provider}`
                : auth.error,
            );
          }
  ...
  // Allow other extensions (e.g. execute-command) to trigger /answer
  // programmatically by emitting the `trigger:answer` event with an
  // ExtensionContext payload.
  pi.events.on("trigger:answer", (ctx: ExtensionContext) => {
    void answerHandler(ctx);
  });
}
