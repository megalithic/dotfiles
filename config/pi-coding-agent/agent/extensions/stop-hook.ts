...
const STOP_CHECK_PROMPT_BASE =
  "Review your last response. Did you complete everything the user asked? If the user asked a question, make sure you answered it in a format they could clearly see and understand. Do not run lat_search or lat_check just because of this review prompt; only use lat tools if you continue work and the actual task requires code or documentation changes. If not, continue working only within the user's requested scope. If the user only asked you to investigate, inspect, check, audit, or report findings, do not fix anything now; report findings and ask before changing anything. If you did complete everything, briefly confirm what was done. If ticket context is provided, suggest the best next ticket(s) by related work first: same parent epic, same plan/task sequence, direct dependents or unblocked siblings, then related scope; in all cases, you must verify that the ticket(s) you are to suggest are in fact still relevant to the work being done (also that the ticket in question hasn't actually already been completed). Fall back to global priority only after related options.";
