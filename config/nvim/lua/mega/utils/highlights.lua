...
---@return string
function mega.highlight.alter_color(color, percent)
...
function mega.highlight.get(group, attribute, fallback)
...
---@return ErrorMsg[]?
function mega.highlight.set(namespace, name, opts)
  ...
  for attr, value in pairs(opts) do
    if type(value) == "table" and value.from then
      local new_attr, err = mega.highlight.get(value.from, value.attr or attr)
      if value.alter then new_attr = mega.highlight.alter_color(new_attr, value.alter) end
...
function mega.highlight.set_winhl(name, win_id, hls)
  local namespace = api.nvim_create_namespace(name)
  mega.highlight.all(hls, namespace)
  api.nvim_win_set_hl_ns(win_id, namespace)
end

function mega.highlight.clear(name)
...
function mega.highlight.all(hls, namespace)
  local errors = mega.fold(function(errors, hl)
    local curr_errors = mega.highlight.set(namespace or 0, next(hl))
    if curr_errors then vim.list_extend(errors, curr_errors) end
    return errors
  end, hls)
  if #errors > 0 then notify(mega.fold(function(acc, err) return acc .. "\n" .. err.msg end, errors, ""), "error") end
end
...
function mega.highlight.plugin(name, opts)
