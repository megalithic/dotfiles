...
local fn = vim.fn
local api = vim.api
local P = require("mega.lush_theme.colors")
local L = P.lsp
...
-- Color Scheme {{{1
-----------------------------------------------------------------------------//
-- if mega.plugin_installed("doom-one.nvim") then
...
-- FIXME:
-- NEW VERSION, fix the invoking code sites
-- local fmt = string.format
-- local fn = vim.fn
-- local api = vim.api
-- local P = require("mega.lush_theme.colors")
...
--   local hl = mega.find(function(part) return part:match(target) end, vim.split(win_hl, ","))
...
-- function M.all(hls)
--   mega.foreach(function(hl) M.set(next(hl)) end, hls)
-- end
