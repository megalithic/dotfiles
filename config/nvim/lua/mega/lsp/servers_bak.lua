return function()
  ...
  local function build_command(server_name, cmd_path, args)
    ...
    if exists then
      ...
      if type(args) == "table" and mega.tlen(args) > 0 and args.fallback ~= nil then
        exists, dir = workspace_has_file(args.fallback)
        if exists then
          logger.debug(fmt("workspace_has_fallback_file: %s", dir))
  ...
  local function lsp_cmd_override(server_name, opts, cmd_path, args)
    args = args or {}
    logger.debug(fmt("lsp_cmd_override: %s %s", server_name, args))
  ...
  local function lsp_setup(server_name, opts)
    logger.debug(fmt("lsp_setup (pre): %s %s", server_name, I(opts)))
  ...
  mason_lspconfig.setup_handlers({
    ...
    elixirls = function(server_name)
      ...
      opts.cmd = lsp_cmd_override(server_name, opts, ".elixir-ls-release/language_server.sh", {
        fallback = string.format("%s/lsp/elixir-ls/%s", vim.env.XDG_DATA_HOME, "language_server.sh"),
      })
