-- @HT: https://github.com/wbthomason/dotfiles/blob/linux/neovim/.config/nvim/lua/start.lua
...
local function cap_path_length(path)
  if string.len(path) > 50 then path = vim.fn.pathshorten(path) end
  return path
end
...
local function longest_elems()
  ...
  for _, section in ipairs(sections) do
    ...
    if title_len > longest_title then longest_title = title_len end
    for _, item in ipairs(section.show) do
      local item_len = string.len(item.disp)
      if item_len > longest_item then longest_item = item_len end
    end
  end
...
local function do_binding(binding)
  if binding.editing then vim.cmd([[ doautocmd User ActuallyEditing ]]) end
...
local function setup_keys()
  ...
  for _, binding in ipairs(keybindings) do
    map(0, "n", tostring(binding.key), "", {
      noremap = true,
      silent = true,
      callback = function() do_binding(binding) end,
    })
  end
end
...
local function start_screen()
  ...
  vim.cmd(
    [[noautocmd silent! setlocal bufhidden=wipe colorcolumn= foldcolumn=0 matchpairs= nobuflisted nocursorcolumn nocursorline nolist nonumber norelativenumber nospell noswapfile signcolumn=no synmaxcol& statusline= filetype=startify]]
  )

  local vc_ok, vc = mega.require("virt-column")
  if vc_ok then vc.setup_buffer({ virtcolumn = "" }) end
  make_sections()
...
--   map(0, "n", "h", "<NOP>", { noremap = true, silent = true })
--   map(0, "n", "l", "<NOP>", { noremap = true, silent = true })
--   map(0, "n", "j", "<cmd>lua require\"mega.start\".handle_j()<cr>", { noremap = true, silent = true })
--   map(0, "n", "k", "<cmd>lua require\"mega.start\".handle_k()<cr>", { noremap = true, silent = true })
--   map(0, "n", "<cr>", "<cmd>lua require\"mega.start\".handle_cr()<cr>", { noremap = true, silent = true })
