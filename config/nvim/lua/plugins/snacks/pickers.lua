...
local function is_jj_root(dir)
  if jj_cache[dir] == nil then jj_cache[dir] = vim.uv.fs_stat(dir .. "/.jj") ~= nil end
  return jj_cache[dir]
end
...
function M.get_root(path)
  ...
  if is_jj_root(path) then return path end

  for dir in vim.fs.parents(path) do
    if is_jj_root(dir) then return vim.fs.normalize(dir) end
  end
