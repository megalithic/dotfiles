...
local function get_shade()
  return require("utils.interop").shade
end
...
local function find_previous_daily_note()
  ...
  -- Filter and sort: find most recent note before today
  local files = {}
  for _, path in ipairs(all_files) do
    local filename = path:match("([^/]+)%.md$")
    if filename and filename < today then
      table.insert(files, { path = path, filename = filename })
    end
  end
  ...
  if #files > 0 then
    return files[1].path, files[1].filename
  end
...
---@return string tasks Formatted task list
local function extract_incomplete_tasks(path)
  local default_tasks = "- [ ] DAILY CHORES #life\n- [ ] Read book chapter #life"

  local f = io.open(path, "r")
  if not f then return default_tasks end
  ...
  if not tasks_section then return default_tasks end
  ...
  if #incomplete == 0 then return default_tasks end
