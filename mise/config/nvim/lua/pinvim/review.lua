...
local function open_neogit_diff(root, diff_mode, range)
  ...
  elseif diff_mode == "range" then
    ...
    require(integration).open("range", range)
    return true
  end
...
--- `range` is a full git rev-range (e.g. "main...HEAD" or "@{u}..HEAD") for
--- range diff_mode; nil for worktree/staged/unstaged/status.
local function open_neogit_review(range, diff_mode)
  ...
  return open_neogit_diff(root, diff_mode, range)
end
...
function M.run(scope, opts)
  ...
  elseif scope == "unpushed" then
    ...
    if not up then
      ...
      -- Unpushed: two-dot against upstream (commits not yet pushed).
      ok = open_neogit_review(up .. "..HEAD", diff_mode or "range")
  ...
  elseif scope == "branch" then
    ...
    if not base then
      ...
      -- Branch: three-dot against base merge-base (only this branch's work,
      -- excludes commits the base advanced independently).
      ok = open_neogit_review(base .. "...HEAD", diff_mode or "range")
      diff_mode = diff_mode or "range"
    end
