...
local function contextsMatch(ctx1, ctx2)
	...
	if not (s1 and w1 and p1 and s2 and w2 and p2) then
		return false
	end

	-- Must match session, window, and pane
	if s1 ~= s2 or w1 ~= w2 or p1 ~= p2 then
		return false
	end
	...
	if pid1 and pid2 and pid1 ~= pid2 then
		return false
	end
...
---Does the source app's focused window correspond to the notification sender?
---Compares the focused window title against the notification title (the sender
---for Messages), ignoring the "Maybe: " contact-suggestion prefix. When no
---Missing title information cannot prove engagement, so the HUD remains enabled.
---@param app table hs.application instance
---@param senderTitle string|nil
---@return boolean
local function sourceWindowMatches(app, senderTitle)
	if not senderTitle or senderTitle == "" then
		return false
	end
	local win = app:focusedWindow()
	local winTitle = win and win:title() or nil
	if not winTitle or winTitle == "" then
		return false
	end
	local sender = senderTitle:gsub("^Maybe: ", ""):lower()
	winTitle = winTitle:lower()
	return winTitle:find(sender, 1, true) ~= nil or sender:find(winTitle, 1, true) ~= nil
end
...
---@param source { bundleID?: string, title?: string }|nil Originating app, for engagement suppression
---@return { state: string, shouldNotify: "full"|"subtle"|"remote_only" }
function M.checkAttention(context, source)
	-- 1. Check display state first (cheapest check, short-circuits everything)
	local displayState = M.checkDisplayState()
	if displayState ~= "awake" then
		return { state = displayState, shouldNotify = "remote_only" }
	end
	...
	-- 4. Source-app engagement: the notification's app is frontmost and the
	-- focused window matches the sender (e.g. already chatting with them in
	-- Messages) - mirror native Messages behavior and stay quiet. App focused
	-- on a different conversation/window still notifies.
	if source and source.bundleID and frontmost and frontmost:bundleID() == source.bundleID then
		if sourceWindowMatches(frontmost, source.title) then
			return { state = "engaged_with_source", shouldNotify = "subtle" }
		end
		return { state = "source_app_other_context", shouldNotify = "full" }
	end

	-- 5. Check if terminal is frontmost
	if not frontmost or frontmost:bundleID() ~= TERMINAL then
		return { state = ATTENTION.TERMINAL_NOT_FOCUSED, shouldNotify = "full" }
	end

	-- 6. Terminal is focused - check if user viewing THIS exact pane
	if context then
...
local function getPhoneNumber()
	...
	-- Check cache first
	local now = os.time()
	if phoneCache.number and (now - phoneCache.timestamp) < cacheTTL then
		return phoneCache.number
	end
...
function M.sendPhone(title, message)
	...
	local success, err = pcall(function()
		hs.messages.iMessage(phoneNumber, fullMessage)
	end)
...
function M.routeNotification(opts, attention)
	local channels = {}
	local cfg = getConfig()
	-- Shared urgency map: same table processor.lua uses for native redirects,
	-- so both pipelines present identically (center+dim for high/critical).
	local urgencyConfig = (C.notifier.urgencyDisplay or {})[opts.urgency]
		or (C.notifier.urgencyDisplay or {}).normal
		or { position = "corner" }
	...
	-- "subtle" = user is engaged with the source (pane/app/conversation) or
	--            screen-sharing, skip local HUD
	...
	if shouldNotify == "full" then
		HUD.toast({
			...
			appBundleID = opts.bundleID or "hal9000", -- HUD renderer handles "hal9000" special case
			duration = cfg.durations[opts.urgency] or 5,
			position = urgencyConfig.position == "center" and "center" or "bottom-left",
	...
	-- Phone: explicitly requested (-p flag) or the urgency map says so (critical)
	local shouldSendPhone = opts.phone or urgencyConfig.phone or false

	if shouldSendPhone then
		local ok, _ = M.sendPhone(opts.title, opts.message)
		if ok then
			table.insert(channels, "phone")
		end
	end
	...
	if shouldSendTelegram and telegram.isReady() then
		...
		if ok then
			table.insert(channels, "telegram")
		end
	end
...
function M.trackQuestion(opts)
	...
	if not questionRetryTimer then
		M.startQuestionRetryTimer()
	end
...
function M.answerQuestion(questionId, title, message)
	...
	if not questionId and title and message then
		questionId = generateQuestionId(title, message)
	end

	if not questionId or not pendingQuestions[questionId] then
		return false
	end
...
function M.send(opts)
	...
	if not opts.title or opts.title == "" then
		return { sent = false, channels = {}, reason = "missing_title" }
	end
	if not opts.message or opts.message == "" then
		return { sent = false, channels = {}, reason = "missing_message" }
	end
	...
	elseif opts.attentionHint == false then
		...
		attention = M.checkAttention(opts.context, { bundleID = opts.bundleID, title = opts.title })
	end
	...
	if opts.question then
		questionId = M.trackQuestion(opts)
	end
