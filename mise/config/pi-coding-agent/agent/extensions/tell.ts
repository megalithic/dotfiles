...
const socketRespondsToPing = (socketPath: string): Promise<boolean> =>
  new Promise((resolve) => {
...
const labelFor = (
  ...
  return `${session}:${window}${pane} — ${cwd}${current}`;
};
...
const discoverCandidates = async (): Promise<Candidate[]> => {
  ...
  if (await fileExists(SOCKET_DIR)) {
    ...
    for (const entry of entries) {
      ...
      if (!bySocket.has(socket)) bySocket.set(socket, undefined);
    }
  }

  const candidates = await Promise.all(
    [...bySocket.entries()].map(async ([socket, manifest]) => {
      if (!(await socketRespondsToPing(socket))) return null;
      return buildCandidate(socket, manifest, currentSocket);
    }),
  );
...
const selectCandidate = async (
  ...
  if (candidates.length === 0) {
    ctx.ui.notify("No running Pi sockets found", "error");
    return null;
  }
  ...
  const title = target?.trim()
    ? `Select Pi instance for "${target.trim()}"`
    : "Select Pi instance";
  const selected = await ctx.ui.select(title, labels);
  if (!selected) return null;
  return candidates.find((candidate) => candidate.label === selected) || null;
};
...
const sendTell = async (
  ...
  const target = await selectCandidate(targetText, ctx);
  if (!target) {
    return { ok: false, error: "No Pi instance selected" };
  }
