	...
	do
		...
		elseif formatting_provider == "null-ls" then
			...
			nls.config({
				...
				sources = {
					fmt.trim_whitespace.with({ filetypes = { "*" } }),
					fmt.prettierd.with({
						filetypes = {
							"javascript",
							"javascriptreact",
					...
					-- 	end,
					-- }),
					fmt.eslint_d, --https://github.com/wesbos/eslint-config-wesbos
					-- fmt.mix,
					fmt.elm_format,
					-- fmt.lua_format,
					diag.shellcheck,
					diag.markdownlint,
					-- diag.mix_credo.with({ filetypes = { "elixir", "eelixir" } }),
					-- diag.selene,
					diag.write_good,
					diag.eslint.with({ command = "eslint_d" }),
				},
			})
