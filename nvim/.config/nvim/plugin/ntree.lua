...
local function git_diff(_, node)
  ...
  bmap("n", "q", "<CMD>exit<cr>", { buffer = bufnr })
  local winnr = mega.floating_window_big(bufnr)
...
local function create_dirs_if_needed(dirs)
  ...
  mega.reduce(dir_split, "", function(directories, dir)
...
local function menu_float(tree, node)
  ...
  local winnr = mega.floating_window_small(bufnr, {})
