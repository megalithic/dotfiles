...
function onInputDeviceChanged(uid, name, scope, _)
  ...
  log.d("User changed unmuted volume for " .. uid .. ": " .. newVolume)
end
...
function updateInputVolumes()
  ...
  for _, device in ipairs(hs.audiodevice.allInputDevices()) do
    ...
    if inputVolumes[device:uid()] == nil then
      ...
      log.d("Setting unmuted volume for " .. device:uid() .. ": " .. inputVolumes[device:uid()])
    end
...
function changeMicrophoneState(mute)
  if mute then
    ...
    local defaultVolumne = preferredVolume -- inputVolumes[defaultInputDevice:uid()]
...
local modifiersChangedTap = hs.eventtap.new({ hs.eventtap.event.types.flagsChanged },
  function(event)
  ...
  -- Public interface
  local module = {}
  module.init = function(modifiers)
    modifierKeys = modifiers or { "fn" }

    log.i("Setting up audio watchers and menubar items with modifiers", hs.inspect(modifierKeys))
  ...
  module.teardown = function(_)
    log.i("Tearing down audio watchers")
