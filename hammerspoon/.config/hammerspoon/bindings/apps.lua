  ...
  for bundleID, app_config in pairs(Config.apps) do
    if app_config.modifier ~= nil and app_config.shortcut ~= nil then
      hs.hotkey.bind(app_config.modifier, app_config.shortcut, function()
        log.df(
          "Toggling or focusing %s (%s) - %s %s (launchMode: %s)",
          hs.inspect(app_config.name),
          bundleID,
          hs.inspect(app_config.modifier),
          hs.inspect(app_config.shortcut),
          hs.inspect(app_config.launchMode)
        )
