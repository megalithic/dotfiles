...
PURE_PROMPT_BACKGROUND_SYMBOL="ﱦ"
...
PURE_GIT_STATUS_RENAMED="${PURE_GIT_STATUS_RENAMED="%F{yellow}»%f"}"
PURE_GIT_STATUS_DELETED="${PURE_GIT_STATUS_DELETED="%F{red}\uf467%f"}" #×
PURE_GIT_STATUS_STASHED="${PURE_GIT_STATUS_STASHED="%F{250}\uf73a%f"}" #⚑
PURE_GIT_STATUS_UNMERGED="${PURE_GIT_STATUS_UNMERGED="%F{orange}=%f"}"
PURE_GIT_STATUS_AHEAD="${PURE_GIT_STATUS_AHEAD="%F{254}↑%f"}"
PURE_GIT_STATUS_BEHIND="${PURE_GIT_STATUS_BEHIND="%F{254}↓%f"}"
PURE_GIT_STATUS_DIVERGED="${PURE_GIT_STATUS_DIVERGED="%F{orange}⇕%f"}"
...
prompt_pure_git_status() {
  ...
  if [[ -n $git_status ]]; then
    print -n "$PURE_GIT_STATUS_PREFIX$git_status$PURE_GIT_STATUS_SUFFIX"
  fi
}
...
prompt_pure_preprompt_render() {
  ...
  # Set the path (optionally, truncated).
...
prompt_pure_async_callback() {
  ...
  case $job in
    ...
    prompt_pure_async_git_dirty)
      ...
      # ORIGINAL/BASIC DIRTY INDICATOR
      # local prev_dirty=$prompt_pure_git_dirty
      # if (( code == 0 )); then
      #   unset prompt_pure_git_dirty
      # else
