{ pkgs, ... }:
let
  # devenv ships its own bundled nix that fetches `github:` flake inputs via a
  # libgit2 git-clone. Our global gitconfig rewrites https://github.com/ to
  # ssh://git@github.com/ (url.insteadOf), so that clone is forced onto ssh and
  # fails with "authentication required but no callback set". devenv only reads
  # gitconfig from $HOME (it ignores GIT_CONFIG_GLOBAL / NIX_USER_CONF_FILES), so
  # we run it under a sandbox $HOME that mirrors the real one via symlinks but
  # drops the github insteadOf rewrite from .gitconfig.
