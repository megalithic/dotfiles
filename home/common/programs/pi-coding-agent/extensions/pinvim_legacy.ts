 ...
 *
 * Legacy live-context bridge for current nvim-aware context injection.
 * Receives editor state from bridge.ts via pi.events bus — no socket of its own.
 *
 * Flow:
 *   nvim → bridge.ts socket → pi.events('pinvim_legacy:editor_state') → this extension
 *
 ...
 *   Reference: @config/nvim/after/plugin/pi_legacy.lua
 *
...
const updateStatus = (): void => {
  ...
  try {
    if (!ctx.hasUI) return;
    ctx.ui.setStatus("pinvim_legacy", formatStatus(editorState));
  } catch {
    latestCtx = null;
  }
};
...
export default function (pi: ExtensionAPI): void {
  ...
  pi.on("before_agent_start", async () => {
    ...
    return {
      message: {
        customType: "pinvim-legacy-live-context",
        content,
        display: false,
      },
    };
  ...
  // Register /pinvim-legacy-info command
  pi.registerCommand("pinvim-legacy-info", {
    description: "Show pinvim legacy status: socket path, editor state, last update",
