...
const parseArgs = (
  args: string[],
): { options: PreviewOptions; content: string[] } => {
...
const buildPreviewCommand = (
  ...
  const resolvedMode =
    options.mode === "auto" ? detectPreviewMode() : options.mode;
...
const runPreview = async (
  ...
  if (!options.html && !isInTmux()) {
    ctx.ui.notify(
      "Preview requires tmux. Run pi inside tmux to use this command (or use --html).",
      "error",
    );
    return;
  }
  ...
  if (options.html) {
    ...
    ctx.ui.notify(
      "No content provided. Use /preview --help for usage.",
      "error",
    );
    return;
  }
  ...
  if (result.code !== 0) {
    const errorMsg =
      result.stderr?.trim() || result.stdout?.trim() || "preview-ai failed";
  ...
  if (options.html) {
    ...
    const label = options.htmlNoOpen
      ? `Rendered (no-open): ${path}`
      : `Opened in browser: ${path}`;
    ctx.ui.notify(label, "info");
    return;
  }

  const typeLabel =
    options.type && options.type !== "auto" ? options.type : "content";
  const resolvedMode =
    options.mode === "auto" ? detectPreviewMode() : options.mode;
