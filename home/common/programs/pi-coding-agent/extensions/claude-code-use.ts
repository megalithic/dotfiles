...
function rewriteAnthropicToolChoice(
  ...
  if (visibleToolName) {
    return visibleToolName === toolChoice.name
      ? toolChoice
      : { ...toolChoice, name: visibleToolName };
  }
  if (disableFiltering) {
    return toolChoice;
  }
...
function filterToolsForClaudeCode(
  ...
  if (Array.isArray(payload.tools)) {
    ...
    tools = payload.tools.flatMap((tool) => {
      ...
      const nextName =
        visibleName ?? (disableFiltering ? originalName : undefined);
