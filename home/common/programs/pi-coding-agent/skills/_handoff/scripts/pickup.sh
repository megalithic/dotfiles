...
list_handoffs() {
  ...
  if [[ -n $session_filter ]]; then
    ...
    for f in "$session_dir"/*.md; do
      ...
      local title
      title=$(rg -m1 "^# Handoff:" "$f" 2>/dev/null | sed 's/^# Handoff: //' || echo "(untitled)")
      echo "  ${basename}  ${title}"
    done
  else
    # List all sessions
    for session_dir in "$HANDOFFS_DIR"/*/; do
      ...
      local total
      total=$(ls "$session_dir"/*.md 2>/dev/null | wc -l | tr -d ' ')
      if [[ $total -gt 5 ]]; then
...
show_bookmark_validation() {
  ...
  local handoff_bookmark
  handoff_bookmark=$(rg -m1 "^\*\*Bookmark:\*\*" "$handoff_file" 2>/dev/null | sed 's/\*\*Bookmark:\*\* //' || echo "")
...
show_current_todos() {
  ...
  for f in "$todos_dir"/*.md; do
    ...
    # Parse with jq
    local status
    status=$(echo "$json_block" | jq -r '.status // "open"' 2>/dev/null || echo "open")
    local title
    title=$(echo "$json_block" | jq -r '.title // "(untitled)"' 2>/dev/null || echo "(untitled)")
