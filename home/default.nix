...
in {
  ...
  xdg.dataFile."hammerspoon/nix_path.lua".text = ''
    -- Auto-generated by Nix on darwin-rebuild - DO NOT EDIT
    -- Provides Nix binary paths and env vars for Hammerspoon's injection (see overrides.lua)
    NIX_PATH = "${config.home.profileDirectory}/bin:/run/current-system/sw/bin"

    -- Environment variables that spawned processes need
    -- NOTE: These mirror values from hosts/megabookpro.nix environment.variables
    NIX_ENV = {
      NOTES_HOME = "${notes_home}",
      OBSIDIAN_HOME = "${notes_home}",
      NVIM_DB_HOME = "${nvim_db_home}",
      DOTS = "${dots}",
