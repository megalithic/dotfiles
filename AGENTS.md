...
Only push when the user explicitly requests it (e.g., "push it", "push to
remote", "go ahead and push").
...
Only SSH when the user explicitly requests it (e.g., "ssh in and check logs",
"check the server").
...
Only deploy when the user explicitly requests it (e.g., "deploy it", "ship it",
"push to prod").
...
Pi can receive messages from Telegram via Hammerspoon. This requires running pi
through `pinvim` or `pisock` wrapper.
...
Telegram messages are forwarded to the `mega` session by default (configured in
`lib/interop/pi.lua`).
...
**`~/.dotfiles/` is ALWAYS the source of truth.** It's version controlled in
git/jj.
...
2. **NEVER symlink FROM nix store TO dotfiles** - flow is always dotfiles → nix
   → home
...
- `PI_CODING_AGENT_DIR` — Current pi agent config directory (e.g.,
  `~/.pi/agent-evirts`)
