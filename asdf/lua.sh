...
if (command -v luarocks &>/dev/null); then
	...
	(! command -v lua-nuspell &>/dev/null) && luarocks install lua-nuspell
fi
...
cd "$build_path/3rd/luamake" && compile/install.sh
cd "$build_path" && (./3rd/luamake/luamake rebuild && log_ok "DONE building sumneko_lua" || log_error "failed to build sumneko_lua")
