...
Helium is the primary browser, consumed as pre-signed, notarized DMG releases from the private `megalithic/helium-macos-releases` repo — CI bakes Widevine in and Developer ID signs the bundle, so the Nix side does no signing or injection of its own.

The package is `pkgs/helium-browser.nix`; the Home Manager module is `home/common/programs/helium-browser/`. (The package replaced a legacy one that injected Widevine into imput's public DMG and ad-hoc re-signed helpers locally; see git history for that machinery.)

## Signed release package

`pkgs/helium-browser.nix` unpacks DMGs from the private `megalithic/helium-macos-releases` repo, whose CI builds Helium with Widevine already injected, then Developer ID signs (team `3ZJ3F5RFBZ`) and notarizes the bundle.

`programs.helium-browser.package` uses it via the `mkChromiumBrowser` default (`pkgs.helium-browser`). The extension update URL reads Chromium prodversion from the framework's `Versions/` directory at eval time via `builtins.readDir`, staying in sync with the package automatically.
