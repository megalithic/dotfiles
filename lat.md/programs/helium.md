...
Since the releases repo went public, the mise world installs Helium declaratively via `[tools."github:megalithic/helium-macos-releases"]` (version `latest`, `asset_pattern` for the arm64 DMG, sha256 verified by mise against the release's `.sha256` asset). Install-failure gotcha: if the tool sits in a stale `MISE_DISABLE_TOOLS` env var, mise drops its options and autodetection excludes `.dmg` assets ("No matching asset found"); see the comment block above the tool entry in `config.toml`. mise does not extract `.dmg` assets — and renames a single-file asset to the tool's short name, dropping the suffix — so the `postinstall` hook `scripts/mise/install-app` finds the payload (by `.dmg` glob, else largest file), mounts it with hdiutil, and swaps the bundle into `/Applications`; it reruns on version changes, which is the upgrade path. `scripts/mise/install-helium` remains an unexposed manual fallback that downloads through the GitHub releases API, verifies the `.sha256`, and copies through hdiutil; it uses an inherited fnox token when available.

## Mise declarative profile setup

The mise world replicates the module's declarative profile/prefs pieces through `mise run setup:helium`, implemented by `scripts/mise/setup-helium`.
