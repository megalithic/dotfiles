...
Since the releases repo went public, the mise world installs Helium declaratively via `[tools."github:megalithic/helium-macos-releases"]` (version `latest`, `asset_pattern` for the arm64 DMG, sha256 verified by mise against the release's `.sha256` asset). Install-failure gotcha: if the tool sits in a stale `MISE_DISABLE_TOOLS` env var, mise drops its options and autodetection excludes `.dmg` assets ("No matching asset found"); see the comment block above the tool entry in `config.toml`. mise does not extract `.dmg` assets — and renames a single-file asset to the tool's short name, dropping the suffix — so the `postinstall` hook `mise/scripts/install-app` finds the payload (by `.dmg` glob, else largest file), mounts it with hdiutil, and swaps the bundle into `/Applications`; it reruns on version changes, which is the upgrade path. `mise/scripts/install-helium` remains an unexposed manual fallback that downloads through the GitHub releases API, verifies the `.sha256`, and copies through hdiutil; it uses an inherited fnox token when available.

## Mise declarative profile setup

The mise world replicates the module's declarative profile/prefs pieces through `mise run setup:helium`, implemented by `mise/scripts/setup-helium`.

It writes the five `External Extensions/<id>.json` files, sets `NSUserKeyEquivalents` plus the Sparkle kill-switch defaults on `net.imput.helium`, installs the `en-US-10-1.bdic` dictionary, and runs [[avwatchd#Setup and migration|avwatchd setup]] for native messaging. It deliberately never touches profile JSON such as "Secure Preferences". `avwatchweb` is local and unpacked, so `bin/helium-launch` loads its repo directory instead of writing it into profile preferences.

The prodversion baked into the extension update URLs is read at runtime from the installed app's framework `Versions/` dir — the mise equivalent of the nix module's eval-time `readDir`. Because it goes stale on Chromium bumps, the helium tool's `postinstall` in `config.toml` chains `mise/scripts/install-app` and `mise/scripts/setup-helium`, so every install or upgrade re-bakes it. Sparkle defaults are additionally declared in `[bootstrap.macos.defaults."net.imput.helium"]` for fresh-machine bootstrap; the task repeats them because package post-install calls setup before `update:system` applies the normal macOS defaults phase. Chromium picks up External Extensions on next launch; deleting a JSON uninstalls and blocklists the extension, so ids dropped from the list are left on disk. `mise run helium:1password-trust` runs the trust checker.

## Home Manager install (removed)

The Home Manager install path is removed with the module; mise's `mise/scripts/install-app` postinstall now owns `/Applications/Helium.app` on both hosts, converged on megabookpro via forced reinstall.
...
`bin/helium-launch` is the single source of truth for launch flags. It loads [[avwatchd#Architecture|avwatchweb]] from `config/avwatchweb`, keeps `--remote-debugging-port=9223` for browser tooling, starts `/Applications/Helium.app/Contents/MacOS/Helium`, then detaches. The fish `helium` function and Hammerspoon hyper+j cold start share it. LaunchServices forwards no flags, so Finder or `open -a` starts omit both local extension loading and CDP attach support until Helium restarts through the launcher. These runtime flags do not modify the signed app bundle, Gatekeeper identity, Widevine, or 1Password pairing. Port 9223 avoids Brave Nightly's 9222 and is no longer an avwatchd dependency.
...
Pi's `web-browser` skill drives the user's running Helium through the `chrome-devtools-attach` MCP server. It never launches another browser automatically.

Before browser work, the skill probes `http://localhost:9223/json/version`. If the port is unavailable, it asks the user to relaunch Helium with remote debugging. Three MCP variants remain configured in `mcp.json`: `chrome-devtools-attach` is enabled and targets Helium on port 9223; isolated `chrome-devtools` and copied-profile `chrome-devtools-profile` are disabled fallbacks used only on explicit request. A `copy-profile.sh` helper copies the daily profile to `~/.cache/agent-web/profile-copy/`. Attach works when Helium starts through `bin/helium-launch` (fish `helium` function or Hammerspoon hyper+j `launchCommand`), which adds `--remote-debugging-port=9223`; a bare LaunchServices launch (`open -a`, Finder) omits the flag and requires a relaunch before browser automation.

The [[lat.md/programs/hammerspoon#Tidewave inspector handoff]] also uses the `chrome-cdp` helper on port 9223, but first verifies that Helium's PID owns the listening socket. Cmd+Shift+C selects the normal localhost app page for the focused Pi worktree's Phoenix port, excludes `/tidewave` routes and the Tidewave IDE on port 9832, foregrounds the selected CDP target, activates Helium, and enters Tidewave Inspect only after the page and toolbar are ready.
