...
Lazy.nvim installs `tpope/vim-obsession`. Running `:Obsession` in a project writes and maintains `Session.vim`; tmux-resurrect's Nvim `session` strategy restores it with `nvim -S` after whole-server recovery.

Language support is declared in `config/nvim/lua/langs/*.lua` and merged into native `vim.lsp.config()` plus conform. Shopify Liquid uses the Shopify CLI language server, Theme Check through that server, Liquid Prettier through project-local npm deps, and the `liquid` Treesitter parser. Biome remains the preferred formatter for supported JS, TS, JSON, and CSS files.

Neovim is fully mise-managed: the `[tools]` entry `neovim = "nightly"` installs nightly via the `vfox:mise-plugins/vfox-neovim` backend (pinned in `mise.lock`), and `~/.config/nvim` is a mise `[dotfiles]` symlink to `mise/config/nvim/` (the sole config tree; the `config/nvim` nix twin, the HM `programs.neovim` module, and the `neovim-nightly-overlay` flake input are removed). The retired HM wrapper's `extraPackages` toolchain never affected the mise binary — LSPs/formatters resolve from the general PATH (nix profile + mise tools). `nvim-pack` uses the main config's plugin inventory and core modules, keeps `init.lua` for core settings, loads pack startup through standard `plugin/*.lua` runtime files, keeps pack state in `nvim-pack-lock.json`, and carries only the floating `:PackFloat` UI from `mc-nvim`. No lazy.nvim is installed; `lua/pack/init.lua` collects specs from the existing `lua/plugins/**` modules, feeds `vim.pack.add()`, and runs `init`/`opts`/`config`/`keys` setup. During automatic setup, it only indexes `setup` on required modules that return tables because some plugin main modules return scalar values. Repos owned by `megalithic` load live from `~/code/oss/<repo>` via `runtimepath` when present (falling back to git), mirroring lazy.nvim `dev`. Main `config/nvim` still uses lazy.nvim, but may bootstrap small native `vim.pack` experiments from `init.lua`; that helper prepends `stdpath("data") .. "/site"` to `packpath` because the Nix-packaged Neovim default packpath omits the native pack install root. These experiments should avoid duplicating plugins already managed by lazy.nvim, such as Firenvim, to prevent extra startup work. `nvim-treesitter` config auto-installs missing parsers from its `parsers` list on startup. Autocmd callbacks prefer current `vim.*` APIs over deprecated aliases; yank highlighting uses `vim.hl.hl_op`, not `vim.hl.on_yank`. LSP progress echo notifications use the current `nvim_echo` progress opts, including a required `source` set to the LSP client name when available. Option mutations avoid chained `vim.opt` infix operations because Neovim nightly rejects multiple operations in one expression. Escape handling does UI cleanup plus opportunistic autosave, but autosave only runs for real file buffers whose parent directory exists, since forcing `:update` on a missing-path buffer raises `E212`.
...
`mise/config/pi-coding-agent/disabled/extensions/_nvim-review.ts` retains the former `/piview [scope] [diff_mode]` Pi command, distinct from the active `/review` pi-review-loop (`agent/extensions/review.ts`). It is off and not linked into Pi.
...
The former `pview [scope]` launcher is retained at `mise/config/pi-coding-agent/disabled/bin/pview` and is off PATH. It launched Pi with `/piview` as the initial command and avoided duplicate review panes.
