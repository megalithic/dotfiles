...
The `/tell` extension replaces the shell-script tell skill for Pi-to-Pi guidance. Bridge ownership is token- and socket-inode guarded: manifests are written only after a successful listen, heartbeats identify stale owners, and shutdown never unlinks another process's socket or manifest. Stale reclaim requires a dead owner and an old heartbeat; ping failure alone never reclaims a socket. Discovery merges manifests and sockets with tmux panes (including orphaned Pi processes), preserving stable candidate IDs, machine/session/window/pane metadata, cwd, pid, state, and reachability. It ignores ephemeral entries for selection, sends `pi.tell.v1` JSON over the generic `bridge.ts` socket, and exposes the `tell_pi` tool so the receiving instance can reply asynchronously. `bin/pi` enables this bridge directly; bridge has no pinvim heartbeat, peer scan, footer status, or Neovim protocol handling. Incoming tell messages are persisted as custom entries, surfaced near the editor through a temporary widget, and mirrored through the same `~/bin/ntfy` path used by the notify extension.
...
Tell target resolution is fail-closed. Explicit targets must confidently match a reachable live candidate; missing, ambiguous, or busy/unreachable targets return an error instead of falling back. Natural hints are quote/punctuation-aware (for example `mega "fix-tell" agent`), ranking is deterministic, and selector options include unique stable IDs. Non-interactive, cancelled, or failed selectors return structured candidates plus an explicit exact-target retry instruction. Never select unreachable/orphaned candidates. Remote resolution uses manifest/tmux inventory over SSH, including hashed `PI_SOCKET` paths, rather than a literal socket-glob first match. The originating/current instance is excluded from implicit selection and from loose target matches, preventing loopback; only an exact self target can select it.
