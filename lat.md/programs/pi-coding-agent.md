...
Turned-off extensions (`execute-command`, `search-sessions`, `pinvim`, and `nvim-review`) live under `disabled/` until re-enabled; the pinvim end-to-end test skill lives there too. Neovim-side pinvim source remains in place, but ordinary Pi no longer loads its socket polling, editor-service RPC, footer updates, or review command.
