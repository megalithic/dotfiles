...
The `heimdall/` extension bridges other pi extensions' exported functions into agent-callable tools, because pi upstream intentionally blocks agents from dispatching slash commands (earendil-works/pi #4754 closed "on purpose", #6010 `executeCommand` not planned). One adapter file per bridged extension plus shared glue in `heimdall/lib.ts`; `heimdall/plannotator.ts` registers `plannotator_review`, `plannotator_annotate`, and `plannotator_annotate_last`, which call `@plannotator/pi-extension/plannotator-browser.ts` session functions directly and block the tool call until the user's browser decision returns as the tool result. User extensions cannot resolve pi-installed packages by bare specifier (node walk-up never reaches `~/.pi/agent/npm/node_modules`), so `lib.ts` imports the package's TypeScript source by absolute path under pi's jiti loader, lazily and guarded: a missing or incompatible plannotator install (verified against 0.27.9) degrades to a clear tool error instead of a startup crash. Tool-call aborts wire to `session.stop()`, and a stopped session (error name `PlannotatorBrowserSessionStopped`) reports "closed without a decision" instead of throwing.
