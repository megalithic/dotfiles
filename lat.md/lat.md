...
The nix build (`pkgs/helium-browser.nix`) injects Widevine and strips all code signatures (nix daemon build users cannot access the keychain). The home-manager activation signs the installed bundle with `Developer ID Application: Seth Messer (3ZJ3F5RFBZ)` inside-out: Sparkle Updater → Sparkle framework → helpers → standalone executables → Helium Framework → main app. The base helper (`Helium Helper.app`) is signed with `--options=runtime,kill,restrict` (no `library`) and a `disable-library-validation` entitlement so Google-signed Widevine CDM loads. The result is a valid bundle seal under a single Developer ID, so Gatekeeper accepts without notarization (no quarantine xattr from rsync install, and unquarantined Developer-ID-signed apps bypass the notarization check). No "Open Anyway" step is needed.
