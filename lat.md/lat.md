...
The main Pi module auto-discovers non-underscore-prefixed `./packages/*.nix` local packages, `.ts` extensions, extension directories, `./agents/*.md`, skill directories under `./skills/`, and `./prompts/*.md` so new simple helpers land in Home Manager by directory shape, not manual list edits. Prefix any auto-discovered file or directory with `_` to temporarily disable it; `extensions/_rtk.ts` and `skills/_handoff/` use this convention to keep source present but unloaded. JSON files in `extensions/` are not auto-discovered unless an extension reads them directly. Helpers that need `node_modules` should usually be installed through Pi package entries in `settings.json`; keep `./packages/*.nix` for local executable adapters or packages that need Nix-specific wrapping. The `agents/` directory is intentionally empty except for `.gitkeep` because package defaults now provide planner/researcher-style subagents.
