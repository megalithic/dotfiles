...
Normal `gpp` opens or focuses a child pi split (registry-allocated under `children/<child-id>/`) and immediately sends cursor/file context from the buffer that was active when the split was requested. Visual `gpp` does the same with selection context. Child split context stays prompt-delivered so its existing start-a-turn behavior is preserved. `<C-p>` (normal and visual) toggles the main parent-owned PiPanel and never spawns a child split. When opening a hidden panel, `<C-p>` captures cursor or selection context before the tmux split steals focus, sends it with attach delivery after the panel becomes visible, and does not trigger a model turn by itself; if a Pi pane is already visible in the current tmux window, `<C-p>` only toggles it closed.
