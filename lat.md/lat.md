...
The `custom-footer.ts` extension replaces the default Pi footer with a starship-backed cwd line plus compact token/model status. The far right of line 2 is reserved for multi-pass routing as `({preset}){active pool}/{model}/thinking_level`, replacing the old preset/provider/model/thinking mix. `multi-pass` status is parsed from `multi-sub.ts`; pool changes are styled distinctly, model stays accent-colored, and preset/thinking stay dim. Caveman status is always suppressed, and MCP status is reduced to ` {active}/{total}` before any remaining extension statuses are merged into the left stats area.
