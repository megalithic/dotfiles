...
NPM-backed Pi helpers use `buildNpmPackage` derivations plus pinned lockfiles under `home/common/programs/pi-coding-agent/packages/`. The main Pi Home Manager module auto-discovers every non-underscore-prefixed `*.nix` file in that directory, builds it with `callPackage`, and symlinks most results into `~/.pi/agent/extensions/<package-name>/`; executable adapter packages such as `pi-acp` can opt out and be installed separately. Prefix a package filename with `_` to keep it available in the repo but inactive. If an upstream release lacks `package-lock.json`, `just update-npm` generates and vendors one from the fetched source before computing `npmDepsHash`. Core `pkgs.pi-coding-agent` is sourced from the shared `nixpkgs` input in `overlays/default.nix`; `just update-npm` updates only active extension package derivations and treats old vendored core wrappers as retired no-ops.
...
Hammerspoon window management uses the custom `wm.lua` grid/geometry path on `hyper+l`; native Tahoe menu tiling is optional on `hyper+w`. `wm.lua` converts the configured `C.grid` 60×20 positions into proportional screen-local frames and applies `C.windowGap` as a pixel inset, so chained movement, center sizing, split tiling, and app layout automation keep spacing and proportions across displays. The WM hypemode auto-exits after 2s of idle time, while chained movement/resize keys use a single local `chainExitDelay` setting of 1.25s after each press. App/window watchers run configured layout rules on launch and window creation/main-window changes, so matching `C.layouts` rules place windows on the configured screen with the same custom frame calculation. Rule precedence inside `placeApp` is per-window: rules with a non-empty title pattern are matched first, and only the first specific match places that window; a catch-all rule (title pattern `nil` or `""`) is used only when no specific rule matched. Exclusion-only rules (`"!fragment"` with no position) suppress placement entirely for matching titles. Ordering of rules inside `C.layouts[bundleID].rules` does not affect outcomes, but specific rules are conventionally listed before the catch-all.
