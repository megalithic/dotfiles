...
Example: Yazi plugins under `yazi/`. Tools flipped to mise ownership (git, jj, bat, eza, ripgrep, fd, karabiner, surfingkeys, starship, fzf, zoxide, atuin, espanso, kitty) live under `mise/config/<tool>/` (or fish `conf.d/` hooks) and are linked by `[dotfiles]` instead. Apps that need live-editable config use out-of-store symlinks into the repo `config/` tree (see [[architecture#Out-of-store config symlinks]]). VS Code is not installed; Neovim is the editor path.
...
Prompt/tool hooks all live in `conf.d/`: `starship.fish`, `fzf.fish` (exports `FZF_DEFAULT_OPTS`/commands), `zoxide.fish`, `atuin.fish`, and `direnv.fish`; the HM starship/fzf/zoxide/direnv modules are removed. The login shell is `/opt/homebrew/bin/fish` (brew formula; nix-darwin fish is removed). Fish fzf Ctrl-T uses fzf's fish token parser, so the current path token becomes `$dir` and `conf.d/fzf.fish` passes `$dir` as fd's explicit search path. Fish defines a `jj` wrapper that runs real `jj` inside jj repos, allows `jj git init`, and falls back to `git` with a hint in plain git repos. `PLUG_EDITOR` is exported as a `hammerspoon://nvim-open` URL so stack-trace links delegate final Neovim target resolution to Hammerspoon.
...
User scripts in `bin/` provide repo workflow shortcuts. Both systems source `~/bin` from `~/.dotfiles/bin`: Home Manager through its generation and mise through a direct directory link. Mise uses `symlink-each` only for mutable `~/.local/bin`.
...
`bin/tmux-process-name` renders automatic window names and pane titles as `<icon> <name>` (adapted from nicknisi's `tmux-smart-name`/`nerdwin`, minus Claude handling). `tmux.conf` passes `#{pane_current_command}`, `#{pane_current_path}`, and `#{pane_tty}` so ps is skipped in the common case; a `node` command is re-identified as pi via `ps -t <tty> -o comm=` matching the `pi` process title; ssh panes keep `ssh:host` naming with foreground (STAT `+`) detection; paths under `<project>/.worktrees/<wt>` render as `project (branch)` (branch falls back to the worktree dir name). A legacy single-tty-arg form keeps live servers sane until config reload. Windows explicitly named by layouts have `automatic-rename` off and are unaffected. The Hammerspoon title parser only reads the `session:window:pane:pid` prefix, so the richer trailing name is compatible.

The `bin/ftm` picker pins running sessions first: MRU-running entries, then remaining sessions by tmux `session_last_attached`. A dim all-dash divider row (sized to the list via `FZF_COLUMNS` on reload, estimated from the tmux client width otherwise) separates them from dead MRU entries, zoxide dirs (default 10, `FTM_ZOXIDE_LIMIT`), and remaining layouts. Enter/Alt-Enter on the divider are no-ops: both binds are fzf `transform` actions that emit `ignore` when the current line is all dashes, so the picker stays open; `try_session` keeps an all-dash backstop check. MRU state lives at `${XDG_STATE_HOME:-~/.local/state}/ftm/recent` and is bumped on every switch/create. Lines are `name<TAB>cwd`, so an entry whose session died but whose dir still exists keeps rendering with its path. The MRU write and post-switch hook launch happen before `switch-client`, because the `C-Space` popup collapses and tmux HUPs the script after switching; post-switch therefore runs detached (`nohup`). Zoxide entries render as `name<TAB>dimmed path` and dedupe by name. A running session wins on selection; normal Enter restores an available ftm snapshot before named layouts, while Alt-Enter bypasses snapshots and named layouts and forces `layouts/default.sh`.

The picker shows an fzf preview (below the list, 65%, toggle with `?`, scroll with `ctrl-u`/`ctrl-d`). The fzf call passes `--height=100%` because the interactive shell exports `FZF_DEFAULT_OPTS` with `--height=25%`, which would otherwise shrink the picker to a quarter of the popup. Running sessions invoke the dependency-free Bun helper `bin/ftm-grid-preview`, which renders one bordered card per live Pi agent in an aspect-scored grid sized to `$FZF_PREVIEW_COLUMNS` by `$FZF_PREVIEW_LINES`; sessions without a Pi agent show their active pane. `pi_panes_for` scans manifest JSON under `$PI_STATE_DIR/manifests/*.info` (socket basenames may be shortened and hashed), rejects records marked `ephemeral`, selects records by their `session` field, unions them with status files, then validates each pid, pane, and `pi` process title through `is_pi_pane` (`ps -t <tty> -o comm=`). The helper polls each target with `capture-pane -e -p -S -200` every 500 ms, removes trailing blank viewport lines, keeps the newest lines that fit each card, and preserves only safe SGR styling. It emits complete newline-delimited frames and uses `CSI 2 J` only when content changes, matching fzf's streaming-preview protocol without raw mode, alternate screens, or cursor positioning. Pi exposes no message-history query over its active socket bridge, so the rendered Pi TUI panes remain the preview of recent Pi messages. Dir entries preview their path; layout entries preview the script; unknown names preview nothing. The `C-Space` popup binding is sized `70%`×`75%` to fit the bottom preview (`mise/config/tmux/tmux.conf`), and direct `ftm` invocations use `fzf --tmux center,70%,75%`.

Running-session rows render bold; non-running entries (layouts, MRU dirs, zoxide dirs) render dim — there is no `*` marker. Running rows also carry a pi status icon using the common three-state agent model (workmux/fleet/claude-squad): red `●` needs attention — blocked on a structured `ask_user_question` or turn ended with an error (states `asking`/`error`); green `●` actively working (state `working`); white outline `○` idle — no prompt yet or turn completed cleanly (states `idle`/`done`). Sessions without a live pi status file render no icon. States come from the `agent-status.ts` pi extension's files (`$PI_STATE_DIR/status/pi-{session}-{window}.status`, line `state pid session`); `print_list` reads that directory once and validates pids with `kill -0` — no sockets or pane captures, so the picker opens instantly and stale files from crashed pis are ignored. Prose questions at turn end are caught by `agent-status.ts`'s trailing-`?` heuristic on the final assistant message and render red `●` (state `asking`). `strip_markers` and the kill/rename sed patterns strip both icons (fzf's `--ansi` already strips the bold/dim styling before selections reach them).

`ftm` stores versioned, base64-safe per-session snapshots under `${XDG_STATE_HOME:-~/.local/state}/ftm/snapshots/<name>`. Each atomic snapshot records session path, window names/indexes/layouts/focus/zoom, pane indexes/cwds/foreground commands/focus, and Pi's `@pi_session_id`; Pi panes restore as `pi --session <uuid>`. `tmux-kill-session` snapshots before Ctrl-K kills. The global tmux `session-closed` hook calls `$HOME/.local/bin/ftm --snapshot-all` to refresh surviving sessions; tmux has already discarded the closed session then, so the pre-kill path owns its final capture.

`ftm` also runs event hooks from `$TMUX_HOOKS` (default `~/.config/tmux/hooks`, sourced from `mise/config/tmux/hooks/`): `pre-create.sh`, `post-create.sh`, and `post-switch.sh`, each executable-gated and failure-swallowed, receiving session name, cwd, and source (`layout`|`zoxide`|`default`|`snapshot`) as args plus `FTM_*` env vars.
...
| bash / fish | **(mise)** shells; fish is default and carries repo helpers; HM modules removed — configs live under `mise/config/fish/` and `mise/config/bash/` |
...
| brave-browser-nightly | **(mise)** Chromium browser; `brew-cask:brave-browser@nightly` owns the app; HM mkChromiumBrowser wrapper removed 2026-08 (managed extensions, `--remote-debugging-port=9222` + flags, ctrl keyEquivalents, spellcheck dicts gone with it) — dock pin points at `/Applications` |
| claude-code | parked until its old dependency is restored |
| codex | OpenAI Codex CLI via `programs.codex` |
| colorsnapper / contexts | **(mise)** macOS GUI utilities; `brew-cask:colorsnapper` + `brew-cask:contexts`; brew-nix HM modules removed 2026-08 |
...
| kitty | **(mise)** terminal emulator; `brew-cask:kitty` + `mise/config/kitty/`; HM module and `config/kitty` twin removed |
| llama-cpp-local | local inference options module (inert — service is the mise `dev.mise.com.megadots.llama-cpp` agent) |
| man | manpage config |
| meetingbar | **(mise)** calendar menu bar app; `brew-cask:meetingbar`; HM package removed 2026-08 |
| mise | tool version manager; standalone binary at `~/.local/bin/mise` (bootstrap.sh installer), global config symlinked from `mise/config/mise/global_config.toml`; the nix module is removed — `mise/config/fish/conf.d/mise.fish` activates it |
...
| yubico-authenticator | **(mise)** Yubico Authenticator GUI app; `brew-cask:yubico-authenticator`; brew-nix HM module removed 2026-08 |
| zoxide | **(mise)** directory jumper; init from `conf.d/zoxide.fish`; HM module removed |
