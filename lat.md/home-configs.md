...
The picker shows an fzf preview (below the list, 65%, toggle with `?`, scroll with `ctrl-u`/`ctrl-d`). The fzf call passes `--height=100%` because the interactive shell exports `FZF_DEFAULT_OPTS` with `--height=25%`, which would otherwise shrink the picker to a quarter of the popup. Running sessions preview the pi agent's pane via `capture-pane -e -p -S -200`, trimmed of trailing blank viewport lines and cut to the newest `$FZF_PREVIEW_LINES` lines (`print_capture_tail`) so the preview shows the latest pi output instead of rendering from the top of stale scrollback; sessions with two or more live pi agents use an experimental stacked preview instead: one section per agent (dim `── window` header with its status icon, then that pane's newest lines, `FZF_PREVIEW_LINES` split evenly; sections separated by a full-width line painted in the popup bg color `#3c474d`, which must stay in sync with `--color=bg:` in `fuzzy_find`) via `pi_panes_for`/`print_stacked_preview` — marked in `bin/ftm` for easy removal, and single-agent sessions never enter it; the pi pane is resolved from the pi manifest (`$PI_STATE_DIR/manifests/pi-{session}-{window}.info`, single-line JSON whose `pane`/`pid` keys are validated against live panes and processes), falling back to the first `node` pane whose process title is `pi` (`is_pi_pane`: `ps -t <tty> -o comm=`, the same trick as `tmux-process-name`), then to the session's active pane; the stacked preview's status-file fallback applies the same title check per window. Pi exposes no message-history query over its active socket bridge, so the rendered Pi TUI pane is the preview of recent Pi messages (fleet by nicknisi previews the same way). Dir entries preview their path; layout entries preview the script; unknown names preview nothing. The `C-Space` popup binding is sized `70%`×`75%` to fit the bottom preview (`mise/config/tmux/tmux.conf`), and direct `ftm` invocations use `fzf --tmux center,70%,75%`.
