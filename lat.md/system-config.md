...
`bootstrap.sh` is also the first mise bootstrap path. It resolves the intended macOS hostname before applying dotfiles or running `mise bootstrap`; `--host <name>` or the prompt can switch `ComputerName`, `LocalHostName`, `HostName`, and NetBIOSName with `scutil`/`defaults`. It enforces the minimum mise version needed for `dotfiles`/`bootstrap`, falls back from the checkout's default remote to `DOTFILES_REPO_URL` when updating the repo (useful when an SSH origin is not authenticated yet), and dies rather than continuing from a stale checkout missing `_mise.toml`. Because the staged config is named `_mise.toml` (inactive on nix-managed machines), the script exports `MISE_OVERRIDE_CONFIG_FILENAMES=_mise.toml` so its mise sub-commands load it without renaming. The final health check is the `doctor` task (`mise/scripts/doctor`), which verifies brew/mise/fish, the login shell, key `[dotfiles]` symlinks, and that no mise tools are missing.
