...
`bootstrap.sh` is also the first mise bootstrap path. It resolves the intended macOS hostname before applying dotfiles or running `mise bootstrap`; `--host <name>` or the prompt can switch `ComputerName`, `LocalHostName`, `HostName`, and NetBIOSName with `scutil`/`defaults`, and pressing Enter accepts the current hostname. In dry-run mode it never reads or prompts for the current hostname: no `--host` skips hostname work, while `--host <name>` only prints the would-run `scutil`/`defaults` commands. It enforces the minimum mise version needed for `dotfiles`/`bootstrap`; when updating an existing checkout fails, it pauses for an interactive choice to retry the configured remote, try the HTTPS `DOTFILES_REPO_URL` fallback, stash local changes and retry HTTPS, or abort. It dies rather than continuing from a stale checkout missing `_mise.toml`. Because the staged config is named `_mise.toml` (inactive on nix-managed machines), the script exports `MISE_OVERRIDE_CONFIG_FILENAMES=_mise.toml` so its mise sub-commands load it without renaming. The final health check is the `doctor` task (`mise/scripts/doctor`), which verifies brew/mise/fish, the login shell, key `[dotfiles]` symlinks, and that no mise tools are missing.
