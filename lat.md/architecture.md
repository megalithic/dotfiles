...
Other inputs include `opnix`, `devenv`, `hunk`, `nh`, `kanata-darwin`, `yazi`, `nix-homebrew` plus the Homebrew taps, and `brew-nix` for cask and `mas` packaging.
...
`config/` holds `nvim/`, `ghostty/`, `kitty/`, `kanata/`, `espanso/`, and `ssh/` (the retired Hammerspoon and tmux Nix twins were deleted after their ownership flips; their active trees are `mise/config/hammerspoon/` and `mise/config/tmux/`). Program modules under `home/common/programs/<tool>/` own the remaining Nix symlink wiring. Config fragments that need Nix-interpolated values are generated into `~/.local/share/...` and sourced from the live config, keeping the editable tree in `config/`.
...
On megabookpro, mise itself is standalone-owned: the binary is installed by `bootstrap.sh`'s installer path to `~/.local/bin/mise`, `~/.config/mise/config.toml` symlinks to the repo global config, and the former nix `programs.mise` module (with its stub global config) is removed. The nix fish module activates the standalone binary; `MISE_DISABLE_TOOLS` is gone — helium and shade-next install through their mise `github:` tools on both hosts. Stale-env trap: a leftover `MISE_DISABLE_TOOLS` (old shells, tmux server env) silently strips a disabled tool's options — asset_pattern/postinstall vanish and github: `.dmg` installs fail with "No matching asset found"; verify with `mise tool <name>` and purge via `tmux set-environment -gu MISE_DISABLE_TOOLS`. The wave-1 in-place flip (see `~/.local/share/pi/docs/.dotfiles/megabookpro-mise-migration.md`) moved these targets to mise ownership and removed their HM modules: bat, eza, ripgrep, fd, worktrunk, surfingkeys, karabiner, jj, git, ssh signing files (allowed_signers, 1Password agent.toml), shade-next (config + hammerspoon fragment), helium-browser, plus the adopted atuin config and the starship.toml link (the starship module keeps `programs.starship.enable` for the fish prompt init until the shells wave). direnv and fzf modules stay nix-owned until the shells wave because their hooks live in HM fish. Hammerspoon flipped after wave 1: the HM module is removed, the app comes from a real Homebrew install (`brew install --cask hammerspoon`; the broken nix-homebrew remnant at `/opt/homebrew` was cleaned out and official Homebrew installed to support it), and `~/.config/hammerspoon` plus the `nix_path.lua` fragment are mise `[dotfiles]` targets. Tmux then flipped fully to mise: `[tools]` and `mise.lock` own the binary version, `mise/config/tmux/` owns `~/.config/tmux`, and `[env]` owns its layout and plugin paths. The Home Manager module, package, generated `nix.conf`, legacy `config/tmux/` twin, and Nix package override are removed. Apply flipped targets with scoped `mise bootstrap dotfiles apply <targets>` — never unscoped while fish/bash/zsh/kanata/`~/bin` remain HM-owned. `mise bootstrap launchd apply` is still skipped on megabookpro because it has no target filter and the `com.megadots.*` agents would duplicate nix-run services; the session indexers stay unloaded pending a move to nicknisi's `sessions` tool.
...
Shell secret loading is mise-owned: the mise zsh/bash configs source the opnix rendered files directly, and interactive fish/bash/zsh load fnox secrets through `fnox activate`. The opnix module still sets `programs.{zsh,bash,fish}` init extensions, but those are inert because no HM shell program module is enabled (the HM fish and bash modules are removed).

On both hosts, `mise/config/fnox/shared.toml` is the committed source of truth: it declares 39 exported environment variables (plus on-demand `env = false` entries) as individual 1Password field references and enables fnox's memory-only daemon cache. The real global config `~/.config/fnox/config.toml` is a GENERATED machine-local file, not a symlink — fnox ignores `import` in the auto-loaded global config, so `mise run setup:fnox` copies the shared body and appends a per-machine `agecache` age provider whose software key lives at `~/.config/fnox/age-cache-key.txt` (mode 0600, generated by `age-keygen`, never committed). `mise run fnox:sync` builds a staged replacement, resolves every onepass-backed secret, writes age-encrypted `sync = { provider = "agecache", ... }` entries, verifies the cache, then atomically installs it; failures leave the active cache unchanged. Fnox resolution prefers the sync cache, so shells and `fnox exec` launchers work offline with no 1Password round-trips until the next sync. `mise run up` invokes `fnox:sync` once explicitly; full machine bootstrap invokes it once through the post-tools hook. `DEEPGRAM_API_KEY` (`op://Crypt/deepgram/api_key`) is among the declared secrets and feeds the Pi Dictate extension; its op item is currently missing, so sync skips it. The providers pin `account = "my.1password.com"` so machines also signed into a work 1Password account still resolve the personal `Crypt` vault. Non-shell Pi and capper launches enter through `fnox exec --replace`; interactive fish, bash, and zsh load secrets through `fnox activate` in their mise-managed shell configs (no rendered files; `~/.config/fnox/secrets/` is gone). `~/.s3cfg` is a mise-managed symlink to the mode-0600 artifact at `~/.local/share/fnox/generated/s3cfg`; `mise/tasks/render-s3cfg` (run automatically at the end of `mise run fnox:sync`; the standalone `fnox:s3cfg` task is retired) renders it from the sanitized `mise/config/s3cmd/s3cfg.template` using three `fnox get` DO Spaces lookups served from the age cache. The obsolete `S3CFG` fnox entry is gone, while both legacy 1Password blob items remain temporarily as rollback safety nets through their clean cutover verification periods. The mise toolset includes `github:str4d/age-plugin-yubikey` for YubiKey-backed age decryption, `pipx:yubikey-manager` for the `ykman` PIV diagnostics CLI, and `pipx:s3cmd` for capper uploads.

`setup:fnox` verifies direct 1Password access when run alone. `fnox:sync` reuses its config-generation path without repeating those checks; the sync itself verifies the onepass source, then one post-sync lookup verifies the refreshed cache.
