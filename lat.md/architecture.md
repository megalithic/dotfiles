...
`[tasks.bootstrap]`; the script only handles shell setup afterward. Keep ordered first-run tasks in `config/mise/config.toml`,
...
`mise/scripts/airupnp-launchd`, which execs the `-static` arm64 binary — the
...
workbookpro's file owns the workbook-only `com.megadots.pi-control-web` LaunchAgent and its `setup:pi-control-web` task; see [[lat.md/programs/pi-control-web#Ownership and deployment]].
...
- `15759652` — YubiKey 5C NFC, fw 5.2.7: GUARDED. Its PIV PIN is blocked and the PUK is unknown; `mise/scripts/setup-yubikey` hard-refuses it via `GUARDED_SERIALS`. The `OP_SERVICE_ACCOUNT_TOKEN` blob can be decrypted by the primary key because it targets the fleet recipient set, so this key can be PIV-reset and re-enrolled as an offline backup mirror whenever convenient.
...
4. Re-encrypt existing blobs to the grown recipient set: `fnox --profile bootstrap reencrypt -p age` (needs a connected key that can decrypt the current blob), then verify `fnox --profile bootstrap get OP_SERVICE_ACCOUNT_TOKEN` and `op whoami`.

The combined `yubikey-identity.txt` (the age provider `key_file`) holds one identity line per fleet member, so decryption works with whichever key is plugged in; the per-serial stubs are the committable records. Losing a key costs nothing cryptographically: any surviving recipient decrypts, and the canonical token lives in 1Password at `Crypt/OP_SERVICE_ACCOUNT_TOKEN/credential`. As a final fallback, mint a new `ops_...` service account with READ access to Crypt and store it with `fnox --profile bootstrap --write-profile bootstrap set OP_SERVICE_ACCOUNT_TOKEN --provider age`. Remove a lost key's recipient from the config and re-encrypt to evict it.
