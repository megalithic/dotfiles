...
On workbookpro, `mise/config/fnox/config.toml` (linked to `~/.config/fnox/config.toml`) declares 39 exported environment variables (plus on-demand `env = false` entries) as individual 1Password field references and enables fnox's memory-only daemon cache. `mise run setup:fnox` creates the config link when missing, checks 1Password CLI vault access, and verifies `APPLE_TEAM_ID` without printing its value. `DEEPGRAM_API_KEY` (`op://Crypt/deepgram/api_key`) is among the declared secrets and feeds the Pi Dictate extension. Its providers pin `account = "my.1password.com"` so machines also signed into a work 1Password account still resolve the personal `Crypt` vault. Non-shell Pi and capper launches enter through `fnox exec --replace`; interactive fish, bash, and zsh load secrets through `fnox activate` in their mise-managed shell configs (no rendered files; the render task and `~/.config/fnox/secrets/` are gone). `~/.s3cfg` is a mise-managed symlink to the mode-0600 artifact at `~/.local/share/fnox/generated/s3cfg`; `mise run fnox:s3cfg` renders it from the sanitized `mise/config/s3cmd/s3cfg.template` using the three relevant DO Spaces fnox fields without putting values in argv or the repo. The obsolete `S3CFG` fnox entry is gone, while both legacy 1Password blob items remain temporarily as rollback safety nets through their clean cutover verification periods. The mise toolset includes `github:str4d/age-plugin-yubikey` for YubiKey-backed age decryption, `pipx:yubikey-manager` for the `ykman` PIV diagnostics CLI, and `pipx:s3cmd` for capper uploads.
