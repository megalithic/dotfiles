...
The mise 2026.6.12 audit verified Homebrew casks for every tracked GUI app. Declarative app-only casks cover Discord, Handy, MeetingBar, ColorSnapper, Contexts, Slack, Proton Drive, Proton VPN, Yubico Authenticator, and 1Password. Hammerspoon and Espanso retain real-Brew hooks because their casks ship binaries. Raycast also uses the real-Brew `post-packages` hook: its cask URL ends in an extension-less `download?build=arm` path, and the backend infers archive type from the URL filename (magic-byte sniffing covers only zip), so it mistakes the DMG for a raw binary and fails with `app artifact 'Raycast.app' was not found`. Ghostty, IINA, Inkscape, Obsidian, MailMate, OBS beta, and Kitty have unsupported extra artifacts; Zoom and Okta Verify require package installation, with Okta's privileged postinstall remaining Nix-owned. Brave Nightly needs its Nix Chromium wrapper and flags; Tidewave has no cask; Helium must use [[helium#Helium browser|its authenticated private-release installer]]. Do not replace any special path with `brew-cask:` without re-auditing backend support and matching its activation semantics.
