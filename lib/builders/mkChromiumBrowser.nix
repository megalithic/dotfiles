...
let
  ...
  browserConfig =
    ...
    let
      ...
      darwinDir =
        if cfg.applicationSupportDir != null then
          cfg.applicationSupportDir
        else if cfg.bundleId != null then
          cfg.bundleId
        else
          browser;
      linuxDir = browser;
      ...
      configDir =
        if pkgs.stdenv.isDarwin then
          ...
          "${config.xdg.configHome}/${linuxDir}";
    ...
    lib.mkIf cfg.enable {
      ...
      home.file =
        lib.listToAttrs (
          (map extensionJson (cfg.extensions or [ ])) ++ (map dictionary (cfg.dictionaries or [ ]))
        )
