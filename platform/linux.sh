...
if [ -f "/etc/debian_version" ]; then
  ...
  sudo apt-get update
  sudo apt-get -y install linux-headers-$(uname -r) build-essential autoconf m4 libncurses5-dev libwxgtk3.0-gtk3-dev libgl1-mesa-dev libglu1-mesa-dev libpng-dev libssh-dev unixodbc-dev xsltproc fop libxml2-utils libncurses-dev openjdk-11-jdk ninja-build gettext libtool libtool-bin autoconf automake cmake g++ pkg-config unzip xclip lib32readline-dev libreadline-dev dirmngr gpg curl fd-find fish zsh exa shellcheck ripgrep libgsl-dev && log_ok "DONE installing linux deps" || log_error "failed to install linux deps"
