...
Acceptance (increment 3): 10. Prefer a saved exact app URL and exclude Tidewave routes. DONE. 11. Fail closed for ambiguous same-port app tabs. DONE. 12. Run `Page.bringToFront`, then focus Helium, wait for a visible complete page and ready toolbar, and click Inspect. DONE. 13. Reload Hammerspoon and validate the live :4300 tab through CDP without synthesizing the final hotkey. DONE. 14. User runs the final Cmd+Shift+C test from the `sm-spp-enable-auth` tmux pi. DONE.

Automated validation (2026-09-16): `bin/hs-reload` completed; a Lua harness passed Tidewave-route exclusion, unique normal-page selection, exact saved-URL selection, unmatched ambiguity rejection, and duplicate exact-URL rejection. The first manual attempt exposed tmux rewriting tab separators as underscores in Hammerspoon's `display-message` output; the parser now uses printable pipe separators. Live Hammerspoon validation resolves the focused pane, and the intended `%414` pane passes the Pi-process, bridge-manifest, Tidewave-connected, worktree, and port-4300 checks. Live CDP found one normal :4300 target at `http://localhost:4300/provider/login`; `Page.bringToFront` returned `{}`, Hammerspoon activated Helium, and the page reported `readyState=complete`, `visibility=visible`, a Tidewave toolbar, and an Inspect button. Review follow-up confirmed robust shell quoting with adversarial PATH values, Hammerspoon resolution of mise's `node` and `tmux`, positive Helium PID ownership of :9223, the unchanged exact URL, and a rendered, enabled, visible Inspect button. The first live hotkey attempt foregrounded the correct `/provider/login` tab, then the outer guard caught `hs.json.encode(target.url)` because Hammerspoon only accepts tables; encoding `{url=target.url}` now carries the URL into JavaScript without changing target validation. The second live attempt again foregrounded the correct tab, then CDP evaluation failed before the click because validation had already declared the injected top-level `const expectedUrl` in the persistent page context. The injected script now runs inside an IIFE, and the shell runner returns bounded merged stderr for CDP failures instead of reducing them to `nil`. Repeat evaluation and stderr-capture harnesses pass. The third live attempt entered Inspect mode correctly (result=active, "Tidewave Inspect ready").

## Increment 4: copied-prompt relay

Tidewave's toolbar "Copy prompt" only writes to the clipboard; nothing consumed it. After Inspect activates, `pidewave.lua` now arms a five-minute `hs.pasteboard.watcher` that forwards clipboard changes containing `<user_prompt>` to the bound pane's bridge socket as `pi.control.v1` `message.send` (`mode: follow_up`, `from: tidewave`), via a new generic `sendPayload(socketPath, payload)` on `lib/interop/pi.lua`'s pooled connections. Consecutive duplicates are suppressed; each copy notifies success or failure; the watcher stops on TTL, module stop, or re-handshake. Live validation: dead-socket sends return false, the watcher arms with a TTL timer, and teardown clears all state.

The final failure was below the payload layer. A one-shot Node client sent the same newline-delimited control request, received the bridge's `{ok:true}` response, and delivered it to pane `%414`. Hammerspoon never connected because `hs.socket` passes Unix paths through `NSURL`, which decoded the `%414` pane suffix. `pi.lua` now applies `hs.http.encodeForQuery` before `connect`; the resulting `%25414` URL path resolves to the literal `%414` socket. Live probes also confirmed that CocoaAsyncSocket queues a write made before its asynchronous connect callback and that a disconnected pooled socket reconnects before the next send. After `bin/hs-reload`, both a direct Hammerspoon control request and a clipboard-watcher copy reached `%414` as Tidewave follow-ups.

Acceptance (increment 4): 15. Copied Tidewave prompts reach the bound tmux pi as follow_up messages without manual paste. DONE.
